...
function addItemsSearch(productos, productosId, loading=null){
    ...
    if(productos.length==0){
        ...
        if(!isNotItem){
            ...
            notItem.classList.add("noItem");
            notItem.innerText="No se han obtenido resultados";
    
            viewProducts.appendChild(notItem);
    
            isNotItem=true
        }

    }else productos.forEach(n => {
        
        isNotItem=false;

        const notItem=document.querySelector(".viewProducts .noItem");

        if(notItem) viewProducts.removeChild(notItem);
...
function loadProduct(item, idItem=null){
    ...
    if(auth.currentUser) readUser(auth.currentUser.uid)
        .then(userData => {
...
function openPanelUser(){
    ...
    container.style.animationName="aparecerPanelUser";
        ...
        .then(data => {
            ...
            saveLabel.addEventListener("click", e => {
                ...
                if(update){
                    ...
                    userUpdate.updateProfile({displayName: `${nombreNuevo} ${apellidoNuevo}`})
                        .then(() => {
                            if(getCookie("userMail")!=emailNuevo){
                                updateCookie("userMail", emailNuevo);
                                userUpdate.updateEmail(emailNuevo)
                                    .then(() => loginForUpdate())  
                        ...
                        function verifyPass(){
                            if(!googleAccount){
                                updateCookie("userPass", passNueva);
                                userUpdate.updatePassword(passNueva)
                                    .then(() => loginForUpdate())
                                    .then(() => updateBBDDUser())
                                    .catch(err => console.error(err));
                                }
                    ...
                    function updateBBDDUser(){
                        updateUser(userUpdate, createDocUser(data))
                            .then(() => {
                                closeDialog();
    ...
    openDialog(e => {
        container.style.animationName="desaparecerPanelUser";
        container.style.animationDuration="1s";
        container.style.animationFillMode="forwards";
        setTimeout(() => {
...
const recoverPass = mail => auth.sendPasswordResetEmail(mail);